  ...
  (use-package org-cliplink
    ...
    (with-eval-after-load 'org-capture
      (add-to-list 'org-capture-templates
                  `("b" "Bookmark plus" entry (file+olp+datetree ,(expand-file-name "bookmarks.org" org-directory))
...
#+begin_src emacs-lisp
  ...
  (add-to-list 'org-capture-templates
               `("r" "Run" entry
                 (file+datetree ,(expand-file-name "run-log.org" org-directory))
                 "* %<%A %e %B %Y (W%V)> %^{Duration}p %^{Distance}p %^{Elevation}p %^{Pace}p \n%?"))
