...
#+startup: contents
#+property: header-args :tangle (expand-file-name "init.el" user-emacs-directory) ####+property: header-args :comments link
...
#+begin_src emacs-lisp
  (defun xtr-org-refile-candidates ()
  ...
  (customize-set-variable 'org-refile-targets
                          '((org-agenda-files :maxlevel . 9)
                            (xtr-org-refile-candidates :maxlevel . 9)))
#+end_src
...
#+begin_src emacs-lisp
  ...
  (defun actuator-appt-notify (minutes-until _current-time message)
   ...
   name of the TODO item that is upcoming."
    (actuator-notify (format "Appointment in %s minutes" minutes-until)
                     message))
