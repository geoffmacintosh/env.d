...
#+begin_src emacs-lisp
  (customize-set-variable 'view-read-only t)           ; Open read-only files in view mode
  (global-unset-key (kbd "C-z"))     ; Disable suspend keybinding
  (customize-set-variable 'window-combination-resize t) ; Split windows so that it's proportional-ish
  (customize-set-variable 'undo-limit (* 10 1024 1024)) ; Set a high undo limit (bytes)
  (global-unset-key (kbd "<C-wheel-down>")) ; I hate scroll-resizing fonts
  (global-unset-key (kbd "<C-wheel-up>"))
  (customize-set-variable 'help-window-select t)            ; Select help window by default, so it's easy
                                          ; to kill
  ...
  (customize-set-variable 'jit-lock-defer-time 0)           ; Defer doing font lock if there's input pending
  ...
  (customize-set-variable 'sentence-end-double-space nil)         ; Single space FTW
  ...
  (customize-set-variable 'indent-tabs-mode nil)            ; Never insert tabs with tab key
  (customize-set-variable 'require-final-newline t)
  (save-place-mode +1) ; Save the place the point lived in
  (customize-set-variable 'find-file-visit-truename nil) ; Don't resolve symlinks
  (customize-set-variable 'confirm-kill-emacs 'y-or-n-p) ; Ensures there is a confirmation
  ...
  (customize-set-variable 'bookmark-save-flag 1)            ; Save every time
  (customize-set-variable 'bookmark-set-fringe-mark nil)    ; Those marks drive me mad
  (customize-set-variable 'frame-resize-pixelwise t)        ; Allow resize by pixels, not just columns or rows
  (customize-set-variable 'frame-inhibit-implied-resize t)  ; The frame isn't resized when the contents are
  (global-set-key (kbd "M-=") #'count-words) ; Dumb count words
  (blink-cursor-mode -1)                     ; Remove blink
  (customize-set-variable 'cursor-type 'box)
  (customize-set-variable 'scroll-conservatively 101) ; Move the buffer just enough to display point, but no more
  (customize-set-variable 'scroll-margin 3)           ; Scroll when the point hits the edge of the screen if 0
  (customize-set-variable 'mouse-wheel-scroll-amount '(1))  ; I should fancify this
  (customize-set-variable 'inhibit-startup-message t)       ; No startup message
  (customize-set-variable 'initial-scratch-message "")      ; No scratch message
  ...
  (customize-set-variable 'epg-pinentry-mode 'loopback)     ; Loopback allows Emacs to request passwords in the minibuffer I think
  (customize-set-variable 'package-native-compile t)        ; Allow native compiled packages
  ;;(customize-set-variable 'switch-to-buffer-preserve-window-point t) ;
  ;;(customize-set-variable 'bookmark-fontify nil)
  (customize-set-variable 'sort-fold-case t)                ; Sort regardless of case
  (customize-set-variable 'disabled-command-function 'ignore) ; Don't yell when doing stuff that's been disabled
  (customize-set-variable 'bidi-paragraph-direction 'left-to-right) ; This way is faster to process if you never use other paragraph directions
  (customize-set-variable 'bidi-inhibit-bpa t)
  (global-so-long-mode +1) ; Fancy hooks to deal with big files.
  (customize-set-variable 'proced-auto-update-flag t)       ; Auto-update proced
  (customize-set-variable 'create-lockfiles nil)            ; Lockfiles are bad news
...
#+begin_src emacs-lisp
  ...
  (customize-set-variable 'ediff-diff-options "-w") ;;-w ignores whitespace changes
  (customize-set-variable 'ediff-forward-word-function 'forward-char) ;;testing
...
#+begin_src emacs-lisp
  ...
  (defun actuator-restore-gc-cons ()
    ...
    (when (boundp 'actuator-original-gc-cons-threshold)
      (customize-set-variable 'gc-cons-threshold  actuator-original-gc-cons-threshold))
    (when (boundp 'actuator-original-gc-cons-percentage)
      (customize-set-variable 'gc-cons-percentage actuator-original-gc-cons-percentage)))
...
#+begin_src emacs-lisp :tangle no
  ...
  (customize-set-variable 'nov-text-width 75)
...
#+begin_src emacs-lisp
  ...
  (when (ignore-errors (file-exists-p org-directory))
    (customize-set-variable 'rmh-elfeed-org-files `(,(expand-file-name "feed.org" org-directory))))
#+end_src
...
#+begin_src emacs-lisp
  ;;(customize-set-variable 'org-archive-save-context-info '(time file olpath todo))
#+end_src
...
#+begin_src emacs-lisp
  ...
  (customize-set-variable 'org-refile-use-cache nil) ;; Change back if slow?
...
#+begin_src emacs-lisp
  (customize-set-variable 'org-agenda-start-with-log-mode nil) ;;'(closed clock)
...
#+begin_src emacs-lisp
  ...
  ;;(customize-set-variable 'org-clock-into-drawer "CLOCKING")
...
#+begin_src emacs-lisp
  ...
  (customize-set-variable 'appt-disp-window-function #'actuator-appt-notify)
#+end_src
...
#+begin_src emacs-lisp
  ...
  ;;(customize-set-variable 'org-archive-location "archive/%s_archive::")
#+end_src
...
#+begin_src emacs-lisp
  ...
  ;;(customize-set-variable 'mu4e-get-mail-command (concat (executable-find "mbsync") " --all"))
...
#+begin_src emacs-lisp
  ...
  ;; (when (display-graphic-p)
  ;;   (customize-set-variable 'smtpmail-smtp-user (auth-source-pass-get "user" "smtp.fastmail.com")))
#+end_src

#+begin_src emacs-lisp
  (customize-set-variable 'mml-secure-openpgp-encrypt-to-self t)
  (customize-set-variable 'mml-secure-openpgp-sign-with-sender t)
  (customize-set-variable 'mml-secure-smime-encrypt-to-self t)
  (customize-set-variable 'mml-secure-smime-sign-with-sender t)
  ...
  ;;(customize-set-variable 'message-citation-line-format "On %Y-%m-%d, %R %z, %f wrote:\n")
...
#+begin_src emacs-lisp
  (customize-set-variable 'mm-sign-option nil)
  (customize-set-variable 'mm-verify-option 'known)
  (customize-set-variable 'gnus-buttonized-mime-types '("multipart/signed"))
  (customize-set-variable 'mm-decrypt-option 'known)
        ...
        (condition-case nil
            (delete-process
             (make-network-process
              ...
              :service 80))
          (error t))
        (customize-set-variable 'smtpmail-queue-mail t)
...
#+begin_src emacs-lisp :tangle no
  (customize-set-variable 'browse-url-handlers '(("wikipedia"   . eww )
...
#+begin_src emacs-lisp :tangle no
  ...
  (customize-set-variable 'eshell-prompt-function #'actuator-eshell-prompt)
#+end_src
