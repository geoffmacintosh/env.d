...
:CREATED:  [2022-01-31 Mon 15:38]
:END:
#+begin_src emacs-lisp
  (when (and *mac* (not (fboundp 'play-sound-internal)))
      (defun play-sound-internal (sound)
        "Internal function for `play sound'.

  There is no error checking to make sure that things are handled,
  so be careful."
        (start-process "Sound" nil "afplay" sound)))
#+end_src
