...
#+begin_src emacs-lisp
  (csetq custom-file (expand-file-name "custom-settings.el" user-emacs-directory))
...
#+begin_src emacs-lisp
  ...
  (run-with-idle-timer 1 nil #'delete-selection-mode t)
...
#+begin_src emacs-lisp
  ...
  variable`actuator-last-backup'."
    ...
    (let* ((current-time (time-convert nil 'integer))
           ...
           (backup-command (format "borg create --compression zstd,3 --exclude \"**/.git\" --exclude \"**.DS_Store\" --exclude \"**#*#\" ~/Backup/archive::org-%s %s" (format-time-string "%FT%T%z") org-directory)))

      (when (>= time-since-last-backup actuator-time-between-backups)

        (shell-command backup-command  nil "*Actuator Backup Error Log*")

        (if (= actuator-last-backup 0)
            (message "Backup up org directory. No backup this session so far.")
          (message (format "Backing up org directory. %s minutes since last backup."
                           (/ time-since-last-backup 60)))))
      (setq actuator-last-backup current-time)))
#+end_src
...
* Org Weather
  #+begin_src emacs-lisp
    ...
    (when *linux*
      (run-at-time "7:00" nil (lambda () (run-with-idle-timer 15 nil #'actuator-org-weather)))
    ...
    (add-hook 'midnight-mode-hook #'actuator-set-up-weather-check-daily)
  #+end_src
