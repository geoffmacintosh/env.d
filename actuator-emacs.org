...
#+begin_src emacs-lisp
  (defvar actuator-path `("/usr/local/sbin"
                          "/usr/local/bin"
                          "/opt/local/sbin"
                          "/opt/local/bin"
                          "/bin"
...
#+begin_src emacs-lisp
  (csetq org-agenda-text-search-extra-files (file-expand-wildcards (expand-file-name "*.org_archive" org-directory)))
#+end_src
...
#+begin_src emacs-lisp
  ...
  (defvar actuator-backup-create-command "borg init --encryption=none")

  (defvar actuator-backup-dir (directory-file-name "~/Backup/"))
#+end_src
...
#+begin_src emacs-lisp
  (defvar actuator-clean-backup-command "borg prune --keep-last=10 --keep-daily=7 --keep-weekly=4 --keep-yearly=1")

  (defvar actuator-compact-backup-command "borg compact")
#+end_src

#+begin_src emacs-lisp
  (defun actuator-clean-backups ()
    "Test"
    (call-process-shell-command
     (concat
      actuator-clean-backup-command
      " "
      (expand-file-name "org" actuator-backup-dir)
      " && "
      actuator-compact-backup-command
      " "
      (expand-file-name "org" actuator-backup-dir))))
#+end_src
...
#+begin_src emacs-lisp
   (defun actuator-backup (path)
      ...
      (let* ((time-stamp (format-time-string "%FT%H%M%S%z"))
             ...
             (backup-path (expand-file-name backup-name actuator-backup-dir)))
        (unless (file-exists-p backup-path)
          (call-process-shell-command
           (concat actuator-backup-create-command " " backup-path) nil nil nil))
...
#+begin_src emacs-lisp
  (defun actuator-backup-org-dir-on-save ()
    ...
    (when (string-equal default-directory (file-name-as-directory (expand-file-name org-directory)))
      (actuator-backup-org)
      (message (format "Backing up %s" (file-name-as-directory (expand-file-name org-directory))))))
