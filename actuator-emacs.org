...
#+begin_src emacs-lisp
  ...
  (defun xtr-minibuffer-try-complete-and-exit ()
    "Exit minibuffer with what is entered, avoiding completion."
    (interactive)
  ...
  (with-eval-after-load 'fido-mode
    (define-key icomplete-fido-mode-map
                (kbd "\C-j") #'xtr-minibuffer-try-complete-and-exit))
#+end_src
...
** TEST Long-running timers
:PROPERTIES:
:CREATED:  [2022-06-07 Tue 22:33]
:END:
#+begin_src emacs-lisp
  ;;(run-at-time "1 week" nil #'xtr-notify)

  (defun xtr-notify ()
    (dired-create-empty-file "~/Desktop/LONGRUN"))
#+end_src
* Modes
:PROPERTIES:
...
#+begin_src emacs-lisp
  ...
  (defvar actuator-backup-dir "~/Backup/"
    "Location for the backup directories to be stored. Must exist. Needs
  a trailing slash and don't not do it.")
#+end_src
