...
#+begin_src emacs-lisp
  ...
  (defvar actuator-backup-dir "~/Backup/"
    "Location for the backup directories to be stored. Must exist.")
#+end_src
...
#+begin_src emacs-lisp
  ...
  If the backup archive hasn't been created, this will initialize
  it first and then make a backup. Should be fast enough to not be an
