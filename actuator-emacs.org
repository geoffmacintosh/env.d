...
#+startup: contents
#+property: header-args :tangle (expand-file-name (concat (file-name-base (buffer-file-name)) ".el") user-emacs-directory) :comments link
...
#+begin_src emacs-lisp :tangle no
  ...
  (load-file (concat (file-name-base (buffer-file-name)) ".el"))
...
#+begin_src emacs-lisp :comments no
  ;;; actuator-emacs.el --- Literate configuration -*- lexical-binding: t; -*-
...
There are three main Emacs files. This one, {{{input-file}}}, makes all the others. [[*Early init][Early init]] and [[*Init][init]] are housed inside this file, but they're purposefullly as minimal as possible, and they tangle out separately. The next few sections before them are the things that need to be on top. Everything else is supposed to be completely location-independent. Emphasis on /supposed/.
...
#+begin_src bat :tangle (if (eq system-type 'windows-nt) (expand-file-name "runemacs.bat" invocation-directory) "no")
...
* Early init
:PROPERTIES:
:header-args: :tangle (expand-file-name "early-init.el" user-emacs-directory) :comments no :mkdirp yes
:END:
...
#+begin_src emacs-lisp
  (when (boundp 'package-quickstart-file)
...
* Init
:PROPERTIES:
:header-args: :tangle (expand-file-name "init.el" user-emacs-directory) :comments no :mkdirp yes
:END:
** Lexical Binding
#+begin_src emacs-lisp
...
#+begin_src emacs-lisp
  (let ((gc-cons-threshold (* 5000 1024 1024))
        (org-file (expand-file-name "actuator-emacs.org" actuator-directory))
        (el-file  (expand-file-name "actuator-emacs.el" user-emacs-directory)))
    (require 'package)
    (package-activate-all)
    (require 'ob-tangle)
    (org-babel-tangle-file org-file nil "emacs-lisp")
    (byte-compile-file el-file)
    (load (concat el-file "c")))
#+end_src
...
#+begin_src emacs-lisp
  (run-with-idle-timer 1 nil #'require 'org-id)
...
#+begin_src emacs-lisp
  ...
  (run-with-idle-timer 5 nil #'actuator-startup-profile)
...
#+begin_src emacs-lisp
  ...
  (with-eval-after-load 'eldoc
    (eval-when-compile
      (declare-function elisp-get-fnsym-args-string@docstring "elisp-mode"))
    ;;(require 'elisp-mode)
...
#+begin_src emacs-lisp
  ...
  (when (package-installed-p 'org-download)
    (run-with-idle-timer 1 nil #'require 'org-download))
...
#+begin_src emacs-lisp
  (when (not (is-win-p))
    (add-to-list 'package-install-selected-packages 'elfeed))
#+end_src
...
#+begin_src emacs-lisp
  (eval-when-compile
    ...
    (declare-function Info-backward-node "info" ())
    (declare-function Info-forward-node "info" ()))
...
#+begin_src emacs-lisp
  ...
  (eval-when-compile
    (declare-function emms-player-mpd-update-all "ext:emms-player-mpd" ())
    (declare-function emms-cache-set-from-mpd-all "ext:emms-player-mpd" ())
    (declare-function emms-player-mpd-connect "ext:emms-player-mpd" ())
    (declare-function emms-mode-line-mode "ext:emms-mode-line" (arg))
    (declare-function emms-playing-time-display-mode "ext:emms-playing-time" (arg))
    (declare-function emms-smart-browse "ext:emms-browser" ())
    (declare-function emms-all "ext:emms-setup" ())
...
#+begin_src emacs-lisp :comments no
  (provide 'actuator-emacs)
  ;;; actuator-emacs.el ends here
#+end_src
