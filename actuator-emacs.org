...
#+begin_src emacs-lisp
  ...
  I actually don't know what STRING is. BACKEND can be some version of html or latex."
    (pcase backend
      ((or `jekyll `html `md)
...
#+begin_src emacs-lisp
  (defun actuator-appt-notify (minutes-until _current-time message)
   ...
   name of the TODO item that is upcoming."
    (declare-function actuator-notify "actuator-emacs" (title message))
...
#+begin_src emacs-lisp
  ...
  (csetq org-contacts-files '("contacts.org" "new-contacts.org" "sas-students.org"))
#+end_src
