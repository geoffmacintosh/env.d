...
#+begin_src emacs-lisp: tangle no
  (add-hook 'org-capture-before-finalize-hook #'actuator-org-insert-heading-timestamp)
#+end_src
...
#+begin_src emacs-lisp
  (with-eval-after-load 'org-capture
    (add-to-list 'org-capture-templates
                 `("b" "Book" entry
                   (file+headline "books.org" "Unwatched")
                   "* %^{Title}p%^{Year}p%^{Series}p\n%?"
                   :empty-lines 1
                   :kill-buffer t)))
#+end_src
** Calendar
...
:END:
Local Variables:
eval: (add-hook 'org-babel-post-tangle-hook #'actuator-regen-init)
eval: (add-hook 'org-babel-pre-tangle-hook #'widen)
End:
